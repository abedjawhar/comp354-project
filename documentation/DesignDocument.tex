...
\begin{itemize}
\item The user module is a facade to the whole subsystem, and is called upon by the MVC controllers to pass the calls to the lower level layers.

\item The account module handles account validation and verification, then passes the modification calls to the DAOs. Through the user package, it calls upon validation of authentication and authorisation of account modification calls.

\item The auth module is the model which handles user authentication and session management, and is called upon by the user facade when other layers require a validation of user authentication. It also uses the DAOs to fetch the users from the database to validate the password and username during a login.

\item The DAO module uses the SQLite module to create and return library DAOs to be used by the upper layers.

\item The SQLite module provides connections to the underlying database.
