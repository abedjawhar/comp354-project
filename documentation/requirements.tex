...
% first name last name, username password
    ...
    \begin{itemize}
        \item The following attributes on the user account  must be between 4 and 16 characters long: username, password, first name, and last name.
        \item \textit{Reasoning:} We do not want usernames and passwords to be too short for security concerns, nor do we want them to be too long, for data size concerns. We also require a first name and last name for security reasons, as our app accesses sensitive bank data.
    \end{itemize}
\end{itemize}
...
\begin{usecase}{Create User Account}
    ...
    \item User: Wants fast and easy account creation, clear and comprehensible display of account requirements.
    \item Company: Wants user interests to be fulfilled, wants to prevent erroneous input, wants fast communication with the local account database as well as failure tolerance when dealing with database manipulation. }
    ...
    \item User enters a username, first name, last name, email, phone number, and address and password.
    ...
    \item The System will move to the login menu after successful account creations.
    \item \textbf{Alternative Flow - Unmet account business rule:} The System will notify the user of an unmet account requirement.
    \item \textbf{Alternative Flow - Already used username:}the System will notify the user if his username is already used by an existing user account.System}
...
\begin{usecase}{Sort Transactions by Any Attribute}
    ...
    \addrow{Pre-Conditions}{User has created and logged in a user account.}
    \addrow{Success Guarantee}{User can sort transactions by selecting one attribute. Default sorting is ascending order by alphabet or value of number. Click the same attribute twice will sort in descending order.}
    \addmulrow{Main Success Flow}{
    ...
    \addrow{Alternate Flows}{the transactions with the given attribute are displayed in an sorted order based on the selected attribute.}
    \addrow{Priority}{High}
    \addrow{Traces to Test Cases}{}
\end{usecase}
...
\begin{usecase}{Categorize Transaction}
    ...
    \addrow{Description} {This use case describes how a user can categorize their transactions with specific labels in order to sort their spending and better manage where most of their money is being spent }
     ...
     \addrow{Alternate Flows}{User will have organized his transactions into specific categories of his choosing for easier viewing. The application will save these new categorized transactions.}
     \addrow{Priority}{High}
     \addrow{Traces to Test Cases}{}
...
\begin{usecase}{Filter Transactions by Date Range}
    ...
    \item Company: Wants to optimize the ease in which a user can track their spending between specific dates and time periods.}
    \addrow{Pre-Conditions}{User has created and logged in a user account.}
    ...
    \addmulrow{Exceptions}{
    \item User has not added any account.
    \item User has no transaction record.
    \item User selects invalid or impossible range of dates.
    }
    \addmulrow{Alternate Flows}{
    \item Transactions that were made between the two selected dates are displayed.
    \item Error message displays that dates selected are invalid.
    }
    \addrow{Priority}{Medium}
    \addrow{Traces to Test Cases}{}
\end{usecase}

\begin{usecase}{Search Transactions by Category}
    \addrow{Case ID}{11}
    ...
    \addmulrow{Alternate Flows}{
    \item Only the transactions in the specified category are displayed in the transactions view.}
    \addrow{Priority}{High}
    \addrow{Traces to Test Cases}{}
\end{usecase}
...
\begin{usecase}{Export Statement as a CSV file}
    ...
    \addmulrow{Main Success Flow}{
      ...
      \item System displays a view asking where the user wants this statement to be saved.
      \item User enters the path of the folder in which the file should be saved.
      \item System generates the statement and saves it in the desired folder of the user's choosing.
    }
    \addmulrow{Exceptions}{
    \item User has no transaction record.
    ...
    \addmulrow{Alternate Flows}{
    \item The CSV file has successfully been generated, and saved in the specified location. It may be blank if the user has no transaction displayed in his view.
    \item An error message indicating a failure to write the file in the specified location should the path be invalid or unauthorised.
...
\begin{usecase}{Email Statement}
    ...
    \addmulrow{Exceptions}{
    \item User has an invalid or empty email in his user account details.
    \item User enters a non-existent path.
    \item User enters an unauthorised path.
    \item User enters a valid path, but a file with the same name as the statement to be generated already exists.
    }
    \addmulrow{Alternate Flows}{
    \item The CSV file has successfully been generated, and sent by email. It may be blank if the user has no transaction displayed in his view.
    \item An error message indicating a failure to send an email, if the email address was empty or invalid.}
    \addrow{Priority}{Low}
    \addrow{Traces to Test Cases}{}
\end{usecase}
